...
class Graticule(VGroup):
    ...
    def create_animation(self, run_time: float = 1.0, lag_ratio: float = 0.05) -> Animation:
        """
...
class LinearGraticule(Graticule):
    ...
    def _create_graticule(self):
        ...
        for i in range(int(min_value / self.secondary_interval), 
                      int(max_value / self.secondary_interval) + 1):
            value = i * self.secondary_interval
            
            # Position along the axis
            pos = (value + self.zero_position * num_units) * self.unit_length * self.direction
...
class GridGraticule(Graticule):
    ...
    def _create_graticule(self):
        ...
        # Vertical grid lines
        for i in range(int(-num_x_units / 2 / self.secondary_interval),
            ...
            if abs(x_pos) <= self.width / 2:
                ...
                line = Line(
                    ...
                    [x_pos, self.height / 2, 0],
                    stroke_width=self.primary_stroke_width if is_primary else self.grid_stroke_width,
                    color=self.color,
                ...
                if is_primary and self.show_labels and value != 0:
                    label = Text(
                        f"{value:.{self.decimal_places}f}" if self.decimal_places > 0 else str(int(value)),
                        font_size=self.label_font_size,
                        color=self.color
                    )
        ...
        for i in range(int(-num_y_units / 2 / self.secondary_interval),
    ...
    def create_animation(self, run_time: float = 1.0, lag_ratio: float = 0.02) -> Animation:
        """
